# Custom ELS Sirens — 1.9.0.0

This revision adds six selectable tone slots, normal/rumbler WAV banks, and per-vehicle beacon/matrix extra controls. The horn now stops the selected siren and restarts its WAV from the beginning on release. The earlier freeze fixes remain: one persistent stereo output mixes the player's siren, horn, manual tone, and AI sirens; WAV decoding and audio-device work stay on a background worker. No vehicle or RAGE native calls run on that worker.
...
- `Tests/` contains 17 regression checks against the actual audio/configuration/playback-rule source and bundled NAudio DLLs. It substitutes a fake output device and small RAGE test doubles, and does not play sound or load the game. New checks cover six-tone cycling, horn stop/restart, both rumbler banks, extra mapping/matrix exclusivity, modifier keys, and configuration save/reload. Native vehicle-extra calls and menu behavior still require in-game verification.
...
2. Select **WAV set to edit > Normal / rumbler OFF**. Assign Tone 1 through Tone 6, Airhorn, and an optional Manual siren. Each has its own volume.
...
There are six selectable main-siren slots per bank. Tone cycling, auto-scan, manual fallback, and AI selection all include slots 5 and 6 and skip unassigned slots. If Manual is unassigned, it uses the next available main tone. Existing four-tone profiles remain valid; new slots default to None. Light-stage tracking remains a separate 1–4-stage setting.
...
The new tone defaults leave ELS's existing scan/cycle/panic keys available. Key values use Windows Forms names, such as `B`, `NumPad1`, or `Control, B`. Modifier chords require the exact Ctrl/Shift/Alt combination. `None` leaves a control unbound.
...
- The output callback mutes when the game heartbeat is more than one second old. There is no background timer iterating the AI collection. An unavailable/stopped output device is retried on the worker at five-second intervals.
...
1. Enter an emergency vehicle and activate all six configured tones repeatedly, including their first use. Check frame pacing and Tone 5/6 keys, manual fallback, cycle, auto-scan, and a profile with only Tone 6 assigned.
